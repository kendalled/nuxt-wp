<template>
<!-- Search Palette TODO: keyboard support, fix animation -->
<div v-if="showModal" class="relative z-50" role="dialog" aria-modal="true">
  <!--
    ...
    <transition
      ...
      <div v-show="showPalette" v-click-outside="vcoConfig" @keydown.escape="closePalette" class="mt-24 mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
        <div class="relative">
          ...
          <input v-model="search" type="text" class="h-12 w-full border-0 bg-transparent pl-11 pr-4 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm" placeholder="Search..." role="combobox" aria-expanded="false" aria-controls="options">
        </div>
        ...
        <ul v-if="resultList.length !== 0" class="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800" id="options" role="listbox">
          ...
          <li v-for="item, ind in resultList" class="cursor-pointer select-none px-4 py-2 hover:bg-blue-600 hover:text-white" :id="`option-${ind}`" role="option" tabindex="-1">
      ...
      listItems: [
        {
          ...
          title: 'Antique Gold Challenge Coins',
          category: 'Coins'
        },
        {
          slug: '/pins/die-struck',
          title: 'Die Struck Lapel Pins',
          category: 'Pins'
        },
        {
          slug: '/coins/antique-silver',
          title: 'Antique Silver Challenge Coins',
          category: 'Coins'
        },
        {
        ...
        },
        {
          ...
          title: 'Antique Copper Challenge Coins',
          category: 'Coins'
        },
        {
